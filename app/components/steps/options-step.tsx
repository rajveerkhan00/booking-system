  ...
  if (!bookingData.selectedCar) {
    return (
      <div className="glass-card-elevated rounded-2xl p-12 text-center animate-fade-in">
        ...
        <h3 className="text-xl font-bold text-white mb-2">No car selected</h3>
        <p className="text-gray-300 mb-6">Please go back and select a vehicle first.</p>
        {onBack && (
          <button
            onClick={onBack}
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-gradient-to-r from-success/10 to-primary/10 border border-success/20 rounded-2xl p-5 flex items-center gap-4 shadow-soft animate-fade-in">
          ...
          <div className="flex-1">
            <p className="text-white font-semibold">Free cancellation up to 48 hours before pick-up</p>
            <p className="text-gray-300 text-sm mt-1">Change your plans? No problem!</p>
          </div>
        ...
        <div className="glass-card-elevated rounded-2xl p-6 animate-fade-in stagger-1">
          <div className="flex items-start justify-between mb-4">
            <div>
              <h3 className="font-bold text-xl text-white">{bookingData.selectedCar.name}</h3>
              <p className="text-gray-400 text-sm font-medium">{bookingData.selectedCar.category}</p>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gradient-to-br from-gray-50 to-gray-100/50 rounded-xl p-3 border border-gray-100">
                  ...
                  <p className="font-bold text-white text-lg">{bookingData.selectedCar.seats}</p>
                </div>
                <div className="bg-gradient-to-br from-gray-50 to-gray-100/50 rounded-xl p-3 border border-gray-100">
                  <div className="flex items-center gap-2 text-gray-400 text-xs mb-1">
                  ...
                  <p className="font-bold text-white text-lg">400 Km</p>
                </div>
                <div className="bg-gradient-to-br from-gray-50 to-gray-100/50 rounded-xl p-3 border border-gray-100">
                  <div className="flex items-center gap-2 text-gray-400 text-xs mb-1">
                  ...
                  <p className="font-bold text-white text-lg">{bookingData.selectedCar.bags}</p>
                </div>
                <div className="bg-gradient-to-br from-gray-50 to-gray-100/50 rounded-xl p-3 border border-gray-100">
                  <div className="flex items-center gap-2 text-gray-400 text-xs mb-1">
                  ...
                  <p className="font-bold text-white text-lg">{bookingData.selectedCar.transmission}</p>
                </div>
              </div>
          ...
          <div className="mt-6 pt-6 border-t border-gray-100 flex justify-between items-center">
            ...
            <div className="text-right">
              <p className="text-gray-400 text-sm font-medium">Car hire fee</p>
              <p className="text-3xl font-bold text-white">{currencySymbol} {carPrice.toLocaleString()}</p>
        ...
        <div className="glass-card-elevated rounded-2xl p-6 animate-fade-in stagger-2">
          <h3 className="font-bold text-xl text-white mb-2">Optional Extras</h3>
          <p className="text-gray-400 mb-6">Payable at pick-up location</p>
...
function ExtraCard({ icon: Icon, title, price, currencySymbol, quantity, onQuantityChange, info }: ExtraCardProps) {
  return (
    <div className={`border-2 rounded-2xl p-5 transition-all duration-300 ${quantity > 0 ? 'border-primary bg-primary/20 shadow-soft' : 'border-white/10 hover:border-white/30'
      }`}>
      <div className="flex items-center gap-3 mb-3">
        <div className={`icon-container icon-container-md rounded-xl ${quantity > 0 ? 'icon-container-primary' : 'bg-white/10 text-gray-400'}`}>
          <Icon className="w-5 h-5" />
        </div>
        <p className="font-bold text-white flex-1">{title}</p>
        {info && (
      ...
      <p className="text-white font-bold text-lg mb-1">{currencySymbol} {price.toLocaleString()}</p>
      <p className="text-gray-400 text-sm mb-4">Per rental</p>
      <div className="flex items-center justify-between bg-white rounded-xl p-2 border border-gray-200">
        <button
          onClick={() => onQuantityChange(quantity - 1)}
          className="w-10 h-10 flex items-center justify-center text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-all"
        >
    ...
    <div className="glass-card-elevated rounded-2xl p-5 space-y-4 animate-fade-in stagger-1">
      ...
      <div className="space-y-3 text-sm">
        <div className="flex justify-between">
          <span className="text-gray-300">Car hire fee</span>
          <span className="text-white font-semibold">{currencySymbol} {carPrice.toLocaleString()}</span>
        </div>
        <div className="pl-4 space-y-1.5 text-gray-400 text-xs">
        ...
        {extrasTotal > 0 && (
          <div className="flex justify-between pt-2 border-t border-white/10">
            <span className="text-gray-300">Extras (at pick-up)</span>
      ...
      <div className="border-t border-white/20 pt-4 flex justify-between items-center">
        ...
        <span className="font-bold text-white text-2xl">{currencySymbol} {totalPrice.toLocaleString()}</span>
      </div>
    </div>
  )
}
